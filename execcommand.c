 ...
 * @path: Colon-separated list of directories in
 * the PATH environment variable.
 *
 * This function attempts to execute the specified
 * command by searching for its
 * executable in the given path. If the command is found,
 * it is executed using
 * the execve system call. If the command is not found,
...
{
strncpy(command_path, dir, sizeof(command_path) -
command_path_length - 1);
strncat(command_path, "/", sizeof(command_path) -
	strlen(command_path) - 1);
command_path_length += 1;
strncat(command_path, command, sizeof(command_path) -
	command_path_length - 1);
...
if (!found)
{
char *sh_argv[] = {"/bin/sh", "-c", command, NULL};
if (execve("/bin/sh", sh_argv, environ) == -1)
{
perror("execve");
}
}
